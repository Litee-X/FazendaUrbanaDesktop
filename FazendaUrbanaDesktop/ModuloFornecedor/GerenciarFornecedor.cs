...
namespace FazendaUrbanaDesktop.ModuloFornecedor
{
    internal class GerenciarFornecedor
    {
        ...
        public DataTable LocalizarFornecedorPorCnpj(string cnpj, ConexaoBanco factory)
        {
            ...
            try
            {
                ...
                {
                    sqlConexaoBanco.Open();
                    string select = "SELECT nome, email, cnpj, endereco FROM Fornecedor WHERE cnpj = @cnpj";
                    SqlCommand comandoSql = new SqlCommand(select, sqlConexaoBanco);
                    comandoSql.Parameters.AddWithValue("@cnpj", cnpj);

                    SqlDataAdapter adapter = new SqlDataAdapter(comandoSql);
                    adapter.Fill(tabelaFornecedores); // Preenche o DataTable com os resultados
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Erro ao localizar fornecedor por CNPJ: " + ex.Message);
            }
